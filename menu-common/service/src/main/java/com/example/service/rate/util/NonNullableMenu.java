...
public class NonNullableMenu {
    ...
    public static Menu checkMenuNotNullAndGet(MenuRepository menuRepository, String menuId, Long restaurantId) {

        Optional<Menu> possibleMenu = menuRepository.getMenuByIdAndRestaurantId(menuId, restaurantId);

        if (possibleMenu.isPresent()) {

            log.info("Receiving a menu with ID = {} for the restaurant with ID = {}",
                                                                                    menuId,
                                                                                    restaurantId);
        ...
        throw new MenuNotFoundException(String.format(
                                                    "The menu with ID = %s for a restaurant with ID = %d hasn't been found",
                                                                                                                   menuId,
                                                                                                                   restaurantId));
    }
}
